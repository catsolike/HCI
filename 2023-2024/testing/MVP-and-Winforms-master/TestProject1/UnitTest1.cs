...
namespace TestProject1
{
    ...
    public class UnitTest1
    {
        ...
        public void LastNameIsNotEmpty()
        {
            ...
            user.Email = "Bob@";
            user.Save();
            Assert.AreEqual(expected: null, actual: user.LastName, message: "The LastName field should be initialized as null");
            Assert.IsTrue(user.ShowFormErrors, "Null last name should trigger validation error");
            Assert.AreEqual(expected: "\nLast Name cannot be empty", actual: user.ErrorMessage, "Empty LastName should have correct error msg");

            user.LastName = "Evans";
            user.Save(); // save is our only trigger for validation right now

            Assert.AreEqual(expected: "Bob", actual: user.LastName, message: "The LastName must be set to a value to remove error");
            Assert.IsFalse(condition: user.ShowFormErrors, message: "A filled in LastName should not trigger an error");
            Assert.AreEqual(expected:null,actual:user.ErrorMessage,"Filled in first name should remove error message");
        }
